...
The script creates a new fasta file called filename_filtered.fasta, which only contains the sequences with a subsequence "close enough" to the aptamer. The sequencing reads that are antisense to the functional RNA have been reverse complemeted, have the prefix "rev" in their name and are found below the sense sequence reads.
